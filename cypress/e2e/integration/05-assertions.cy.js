...
describe("Assertions", () => {
  ...
  it.only('Assertion Practices', () => {
    ...
    const names = ['Ameer', 'Saleh']
    cy.get('[data-identifier="Text Inputs"] input').each(($el, index) => {
      cy.wrap($el).type(names[index]).should('be.enabled')
    ...
    cy.get('#company_dropdown1, #company_dropdown2').each(($el, index) => {
      cy.wrap($el).select(companies[index])
      .should('be.enabled')
      .and('have.value', companies[index])
    })

    cy.get('#company_dropdown1, #company_dropdown2').each(($el, index) => {
      cy.wrap($el).select(companies[index])
      .find('option:selected')
      .should('have.text', companies[index])
    })
  })
});